	...
	function premake.codelite.project(prj)
		...
		for _, platform in ipairs(platforms) do
			for cfg in premake.eachconfig(prj, platform) do
				...
				local compiler = iif(os.is("windows"), iif(platform == "x64", "MinGW ( mingw64 )", "MinGW ( mingw32 )"), "GCC")
				_p('    <Configuration Name="%s" CompilerType="%s" DebuggerType="GNU gdb debugger" Type="%s">', name, compiler, types[cfg.kind])

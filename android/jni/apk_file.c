...
void APKInit()
{
  ...
  fileHelper = (*env)->FindClass(env, "org/orx/lib/APKFileHelper");
  __android_log_print(ANDROID_LOG_DEBUG, "apk",  "class = %d\n", fileHelper);
  jmethodID getInstance = (*env)->GetStaticMethodID(env, fileHelper, "getInstance", "()Lorg/orx/lib/APKFileHelper;");
  __android_log_print(ANDROID_LOG_DEBUG, "apk",  "inst = %d\n", getInstance);
  APKFileClass = (*env)->FindClass(env, "org/orx/lib/APKFileHelper$APKFile");
